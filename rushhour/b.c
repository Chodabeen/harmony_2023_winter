...
state * make_state(state * prev, state * next) {
...
state * pop_state(state_head * queue_state) {
  state * front = queue_state->next;
  front->next->prev = NULL;
  queue_state->next = front->next;
...
int main(int argc, char *argv[]) {
  ...
  state_head checked_state, queue_state;
  ...
  while (1) {
    ...
    switch (op = get_op_code(buf)) {
      ...
      case start:
        scanf("%s", buf) ;
        queue_state.next = make_state(NULL, NULL);
        if (load_game(buf, queue_state.next) == 1) {
            fprintf(stderr, "invalid data\n");
            free (queue_state.next);
            continue;
        }
        update_cells(queue_state.next) ;
        display(buttons, queue_state.next) ;
        gtk_widget_show_all(window);
        gtk_main();
        break;
  ...
  while (queue_state.next != NULL) {
    break;
    state * st = pop_state(&queue_state);
